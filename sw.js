...
self.addEventListener('push', event => {
  let data = { title: '🦊 Kitsune Finance', body: 'Tienes una alerta financiera', icon: './icons/icon-192.png', badge: './icons/icon-192.png' };
...
function gisLoaded() {
  tokenClient = google.accounts.oauth2.initTokenClient({
    client_id: CLIENT_ID,
    scope: SCOPES,
    callback: '', // se define al hacer clic
  });
  gisInited = true;
}

// 2. Función para iniciar sesión (La magia)
function handleSyncAuth() {
...
// 3. Guardar datos en la carpeta oculta de Drive
async function syncLocalToDrive() {
  const localData = localStorage.getItem('kitsune_db'); // Tu clave de datos actual
  if (!localData) return;

  // Buscar si el archivo ya existe en Drive
  const response = await gapi.client.drive.files.list({
    q: "name='kitsune_backup.json' and spaces='appDataFolder'",
    fields: 'files(id)',
  });
  ...
  if (fileId) {
    ...
    // Crear archivo nuevo
    const metadata = {
      name: 'kitsune_backup.json',
      parents: ['appDataFolder']
    };
    // (Aquí se usa una petición multipart para crear el archivo la primera vez)
    // Por simplicidad, este es el concepto de guardado.
  }
